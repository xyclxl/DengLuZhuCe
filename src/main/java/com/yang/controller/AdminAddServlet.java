...
public class AdminAddServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Date date = new Date();
        Timestamp subtime = new Timestamp(date.getTime());
//        String subtime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(time);
        System.out.println(admin_name+"---------"+admin_password+"---------"+admin_email+"---------"+admin_phone+"---------"+admin_sign+"---------"+subtime);
        if(admin_name!=null && !admin_name.equals("") && admin_password!=null && !admin_password.equals("")
                ...
                && admin_sign!=null && !admin_sign.equals("")){
            //把用户名、密码、邮箱、手机添加到数据库中
            AdminInfoService adminService = new AdminInfoServiceImpl();
            AdminInfo admin = new AdminInfo(admin_name,admin_password,admin_email,admin_phone,admin_sign,subtime);
            int getRow = adminService.add(admin);
            if(getRow>0){
                //注册成功
                response.sendRedirect("jsp/AdminSuccess.jsp");
            }else{
                //注册失败
